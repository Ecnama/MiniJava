...
let options =
  [
    ("--show-tokens", Arg.Set show_tokens, " print all tokens and stop");
    ( "--show-tokens-with-loc",
      Arg.Set show_tokens_with_loc,
      " print all tokens with location informations and stop" );
    ("--show-ast", Arg.Set show_ast, " show abstract syntax tree and stop");
    ( "--show-ast-with-loc",
      Arg.Set show_ast_with_loc,
      " show abstract syntax tree with location informations and stop" );
    ("--stop-at-parsing", Arg.Set stop_at_parsing, " parse the program and stop");
    ( "--stop-at-typechecking",
      Arg.Set stop_at_typechecking,
      " typecheck the program and stop" );
    ("--tgc-path", Arg.Set_string tgc_path, " path to tgc; default is ./tgc");
    ("--c-compiler", Arg.Set_string cc, " c compiler; default is cc");
  ]
  ...
  lexbuf.Lexing.lex_curr_p <-
    {
      ...
      Lexing.pos_lnum = 1;
      (* The first line number is 1 and not 0. *)
      Lexing.pos_bol = 0;
      Lexing.pos_cnum = 0;
    };
  try
    if !show_tokens || !show_tokens_with_loc then begin
    ...
    if !show_ast || !show_ast_with_loc then begin
    ...
    match
      Unix.system
        (Printf.sprintf "%s %s -o %s -I%s %s/tgc.o -std=c99" !cc !ofile
           (Filename.chop_extension !ifile)
           !tgc_path !tgc_path)
    with
