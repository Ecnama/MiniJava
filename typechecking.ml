...
type method_type =
  typ list * typ (* Parameters types and return type of a method. *)
...
(** [lookup msg id env] lookups the identifier [id] in the map [env]. If the
    identifier is not present raises an error using the message [msg]. *)
let lookup (msg : string) (id : identifier) (env : 'a SM.t) =
  try SM.find (Location.content id) env
  with Not_found ->
    error id (sprintf "%s %s is undefined" msg (Location.content id))

(** [vlookup id env] lookups the variable [id] in the environment for variables
    (locals or parameters) [env]. *)
...
(** [alookup a env] lookups the attribute [a] in the environment for attributes
    [env]. *)
...
(** [compatible t1 t2 instanceof] returns true iff the type [t1] is compatible
    with type [t2]. For classes, uses the function [instanceof] to decide if
    [t1] is an instance of [t2]. *)
let rec compatible (typ1 : typ) (typ2 : typ)
    (instanceof : identifier -> identifier -> bool) : bool =
...
(** [typ_lmj_to_tmj t] converts the [LMJ] type [t] into the equivalent [TMJ]
    type. *)
...
(** [typ_tmj_to_lmj s e t] converts the [TMJ] type [t] into the equivalent [LMJ]
    type using location starting position [s] and location ending position [e].
*)
...
(** [tmj_type_to_string t] converts the [TMJ] type [t] into a string
    representation. *)
...
let rec type_to_string (typ : LMJ.typ) : string =
  type_lmj_to_tmj typ |> tmj_type_to_string
...
(** [typecheck_call cenv venv vinit instanceof o callee es] checks, using the
    environments [cenv] and [venv], the set of initialized variables [vinit] and
    the [instanceof] function, that * the expression [o] is an object of type
    [t], * the method [callee] belongs to the class [t], * the parameters [es]
    are compatibles with the types of the formal parameters. If [typecheck_call]
    succeeds, the return type of [callee] is returned. *)
let rec typecheck_call (cenv : class_env) (venv : variable_env) (vinit : S.t)
    (instanceof : identifier -> identifier -> bool) (o : expression)
    (callee : identifier) (expressions : expression list) : TMJ.expression =
  let o' = typecheck_expression cenv venv vinit instanceof o in
  match o'.typ with
  | Typ t -> begin
      let _, method_env =
        Location.(clookup (make (startpos o) (endpos o) t) cenv)
      in
      ...
      try
        let expressions' =
          List.fold_left2
            (fun acc formal e ->
              typecheck_expression_expecting cenv venv vinit instanceof formal e
              :: acc)
            [] formals expressions
          |> List.rev
        in
      ...
      with Invalid_argument _ ->
        error callee
          (sprintf "Invalid function call, expected %d arguments, got %d"
             (List.length formals) (List.length expressions))
    end
  | _ ->
      error o
        (sprintf "A class is expected, got %s" (tmj_type_to_string o'.typ))

(** [typecheck_expression_expecting cenv venv vinit instanceof typ1 e] checks,
    using the environments [cenv] and [venv], the set of initialized variables
    [vinit] and the [instanceof] function, that the expression [e] has a type
    compatible with type [typ1]. *)
and typecheck_expression_expecting (cenv : class_env) (venv : variable_env)
    (vinit : S.t) (instanceof : identifier -> identifier -> bool) (typ1 : typ)
    (e : expression) : TMJ.expression =
  let e' = typecheck_expression cenv venv vinit instanceof e in
  if
    not
      (compatible
         Location.(type_tmj_to_lmj (startpos e) (endpos e) e'.typ)
         typ1 instanceof)
  then
    error e
      (sprintf "Type mismatch, expected %s, got %s" (type_to_string typ1)
         (tmj_type_to_string e'.typ));
  e'

(** [typecheck_expression cenv venv vinit instanceof e] checks, using the
    environments [cenv] and [venv], the set of initialized variables [vinit] and
    the [instanceof] function, that the expression [e] is well typed. If
    [typecheck_expression] succeeds, the type of [e] is returned. *)
and typecheck_expression (cenv : class_env) (venv : variable_env) (vinit : S.t)
    (instanceof : identifier -> identifier -> bool) (e : expression) :
    TMJ.expression =
  match Location.content e with
  | EConst (ConstBool b) -> mke (TMJ.EConst (ConstBool b)) TypBool
  | EConst (ConstInt i) -> mke (TMJ.EConst (ConstInt i)) TypInt
  | EConst (ConstFloat f) -> mke (TMJ.EConst (ConstFloat f)) TypFloat
  | EConst (ConstString s) -> mke (TMJ.EConst (ConstString s)) TypString
  | EGetVar v ->
  ...
  | ERUnOp (op, e) ->
      let expected, returned =
        match op with RUOpIncr | RUOpDecr -> (TypInt, TypInt)
      in
      let e' =
        typecheck_expression_expecting cenv venv vinit instanceof expected e
      in
  ...
  | EBinOp (op, e1, e2) ->
      let expected, returned =
        match op with
        | OpBWAnd | OpBWOr | OpBWXOr | OpAdd | OpSub | OpMul | OpDiv | OpRem ->
            (TypInt, TypInt)
        | OpEq | OpNotEq | OpLt | OpLtEq | OpGtEq | OpGt -> (TypInt, TypBool)
        | OpOr | OpAnd -> (TypBool, TypBool)
      in
      let e1' =
  ...
  | EFunOp (op, e1, e2) ->
      let expected, returned = match op with OpMod -> (TypInt, TypInt) in
      let e1' =
        typecheck_expression_expecting cenv venv vinit instanceof expected e1
      in
      let e2' =
  ...
  | EArrayGet (earray, eindex) -> (
      ...
      | TypIntArray -> mke (TMJ.EArrayGet (earray', eindex')) TypInt
      | TypFloatArray -> mke (TMJ.EArrayGet (earray', eindex')) TypFloat
      | _ ->
          error earray
            (sprintf "Array type incorrect, got %s"
               (tmj_type_to_string earray'.typ)))
  | EIntArrayAlloc elength ->
      let elength' =
        typecheck_expression_expecting cenv venv vinit instanceof TypInt elength
      in
      mke (TMJ.EIntArrayAlloc elength') TypIntArray
  | EFloatArrayAlloc elength ->
      let elength' =
        typecheck_expression_expecting cenv venv vinit instanceof TypInt elength
      in
      mke (TMJ.EFloatArrayAlloc elength') TypFloatArray
  | EArrayLength earray -> (
      let earray' = typecheck_expression cenv venv vinit instanceof earray in
      match earray'.typ with
      | TypIntArray | TypFloatArray -> mke (TMJ.EArrayLength earray') TypInt
      | _ ->
          error earray
            (sprintf "Array type incorrect, got %s"
               (tmj_type_to_string earray'.typ)))
  | EThis ->
      mke TMJ.EThis
        (vlookup
           (Location.make (Location.startpos e) (Location.endpos e) "this")
           venv)
  | EObjectAlloc id ->
      clookup id cenv |> ignore;
      mke (TMJ.EObjectAlloc (Location.content id)) (Typ id)

(** [typecheck_instruction cenv venv vinit instanceof inst] checks, using the
    environments [cenv] and [venv], the set of initialized variables [vinit] and
    the [instanceof] function, that the instruction [inst] is well typed. If
    [typecheck_instruction] succeeds, the new set of initialized variables is
    returned. *)
let rec typecheck_instruction (cenv : class_env) (venv : variable_env)
    (vinit : S.t) (instanceof : identifier -> identifier -> bool)
    (inst : instruction) : TMJ.instruction * S.t =
  match inst with
  | ISetVar (v, e) ->
      let typ = vlookup v venv in
      let e' =
        typecheck_expression_expecting cenv venv vinit instanceof typ e
      in
      let vinit = S.add (Location.content v) vinit in
      (TMJ.ISetVar (Location.content v, type_lmj_to_tmj typ, e'), vinit)
  | IArraySet (earray, eindex, evalue) ->
      let array_typ = vlookup earray venv in
      let value_typ =
        ...
        | _ ->
            error earray
              (sprintf "Type mismatch, expected int[] or float[], got %s."
                 (type_to_string array_typ))
      in
      let eindex' =
        typecheck_expression_expecting cenv venv vinit instanceof TypInt eindex
      in
      let evalue' =
        typecheck_expression_expecting cenv venv vinit instanceof value_typ
          evalue
      in
  ...
  | IFor (id1, e1, e2, e3, i3) ->
      ...
      let e1' =
        typecheck_expression_expecting cenv venv vinit instanceof t1 e1
      in
      let vinit = S.add (Location.content id1) vinit in
      let e2' =
        typecheck_expression_expecting cenv venv vinit instanceof TypBool e2
      in
      let e3' = typecheck_expression cenv venv vinit instanceof e3 in
      let i3', vinit' = typecheck_instruction cenv venv vinit instanceof i3 in
      (TMJ.IFor (Location.content id1, e1', e2', e3', i3'), vinit')
  | IExpr e ->
      let e' = typecheck_expression cenv venv vinit instanceof e in
      (TMJ.IExpr e', vinit)
  | IContinue -> (TMJ.IContinue, vinit)
  | ISyso e -> (
      let e' = typecheck_expression cenv venv vinit instanceof e in
      match e'.typ with
      | TypInt | TypFloat | TypString -> (TMJ.ISyso e', vinit)
      | _ ->
          error e (sprintf "System.out.println expects int, float, or string."))
  | IBreak -> (TMJ.IBreak, vinit)

(** [occurences x bindings] returns the elements in [bindings] that have [x] has
    identifier. *)
let occurrences (x : string) (bindings : (identifier * 'a) list) :
    identifier list =
  List.map fst (List.filter (fun (id, _) -> x = Location.content id) bindings)

(** [map_of_association_list entity bindings] creates a map from the association
    list [bindings]. If some identifiers are duplicated,
    [map_of_association_list] raises an [Error] exception, using the string
    [entity] in the error message. *)
let map_of_association_list (entity : string)
    (bindings : (identifier * 'a) list) : 'a SM.t =
  try
    SM.of_association_list
      (List.map (fun (id, data) -> (Location.content id, data)) bindings)
  with SM.Duplicate x ->
    errors (occurrences x bindings)
      (sprintf "%s %s is declared more than once" entity x)

(** [variable_map decls] creates an environment for variables using the
    association list [decls]. *)
let variable_map (decls : (identifier * typ) list) : variable_env =
  map_of_association_list "Variable" decls

(** [method_map decls] creates an environment for methods using the association
    list [decls]. *)
let method_map (decls : (identifier * method_type) list) : method_env =
  map_of_association_list "Method" decls

(** [typecheck_method cenv venv instanceof m] checks, using the environments
    [cenv] and [venv] and the [instanceof] function, that the method [m] is well
    typed. *)
let typecheck_method (cenv : class_env) (venv : variable_env)
    (instanceof : identifier -> identifier -> bool) (m : metho) : TMJ.metho =
  let formals = m.formals and locals = m.locals in

  let mformals = variable_map formals and mlocals = variable_map locals in

  begin try
    let x =
      StringSet.choose
        (StringSet.inter (SM.domain mformals) (SM.domain mlocals))
    in
    errors
      (occurrences x formals @ occurrences x locals)
      "A formal parameter and a local variable cannot carry the same name"
  with Not_found -> ()
  end;

  let venv = SM.addm mformals venv |> SM.addm mlocals in

  let vinit = S.diff (SM.domain venv) (SM.domain mlocals) in
  let body', vinit =
  ...
  TMJ.
    {
      formals =
        List.map
          (fun (id, typ) -> (Location.content id, type_lmj_to_tmj typ))
          m.formals;
...
(** [typecheck_class cenv instanceof (name, c)] checks, using the environments
    [cenv] and [venv] and the [instanceof] function, that the class [name] with
    type [c] is well typed. *)
let typecheck_class (cenv : class_env)
...
(** [class_map decls] creates an environment for classes using the association
    list [decls]. *)
...
(** [create_instancef cmap] creates an [instanceof] function such that
    [instanceof id1 id2] is true iff class [id2] is a parent (direct or
    indirect) of class [id1]. *)
let create_instanceof (cmap : clas SM.t) : identifier -> identifier -> bool =
  ...
  in
  fun id1 id2 -> instanceof (Location.content id1) (Location.content id2)
...
(** [add_method cmap instanceof] completes each class in [cmap] by creating a
    new map where we add to a given class the methods and attributes of its
    parents. If a method in a parent class has the same name than a method in a
    subclass, we check that the later overrides the former. *)
let add_method (cmap : clas SM.t)
    (instanceof : identifier -> identifier -> bool) : clas SM.t =
  let test_compatible_signature ((name, m) : identifier * metho)
      ((name', m') : identifier * metho) : unit =
    let typecheck_params (typ : typ) (typ' : typ) : unit =
      if
        not
          (compatible typ typ' (fun t1 t2 ->
               Location.content t1 = Location.content t2))
      then
        errors [ name; name' ]
          (sprintf
             "Type mismatch in params of overriden method, expected %s, got %s"
    ...
    let typecheck_result (typ : typ) (typ' : typ) : unit =
      if not (compatible typ' typ instanceof) then
        errors [ name; name' ]
          (sprintf
             "Type mismatch in result of overriden method, expected %s, got %s"
             (type_to_string typ) (type_to_string typ'))
    in
    ...
    with Invalid_argument _ ->
      errors [ name; name' ]
        (sprintf
           "A function that overrides another one must have the same number of \
            parameters")
  in
  (*
              ...
              List.filter
                (fun (name, _) ->
                  not
                    (List.exists
                       (fun (name', _) ->
                         Location.content name = Location.content name')
                       c.attributes))
                c'.attributes
              @ c.attributes;
            methods =
...
let typecheck_program (p : program) : TMJ.program =
  ...
  let cenv = add_method cmap instanceof |> SM.map extract_class_type in
  let defs' = List.map (typecheck_class cenv instanceof) p.defs in
